...
def main(ax, read, inisnap, endsnap, Cell,label,color, saveload,diskmass):
    if saveload==False:
        ...
        for i in range(numsnap):
            ...
            np.savetxt(read + 'sd2.dat', sdarr, delimiter=' ', newline='\n')
            np.savetxt(read + 'sfrd2.dat', sfrdarr, delimiter=' ', newline='\n')

    else:

        sdarr = np.loadtxt(read + 'sd2.dat')
        sfrdarr = np.loadtxt(read + 'sfrd2.dat')
    #ax.scatter(sdarr, sfrdarr, color=color, marker='o', s=5, alpha=0.5,label=label)
...
main(ax1,read_new_01Zsun,150, 480, Cell,'G9_Zlow','k',False,300)
main(ax1, read_new_1Zsun,150, 480, Cell,'G9_Zhigh','r',False,300)
main(ax1,read_new_1Zsun_highSN_new,70, 380, Cell,'G9_Zhigh_SN5','magenta',False,220)
main(ax1,read_new_03Zsun_highSN,70, 380, Cell,'G9_Zmid_SN5','orange',False,220)
main(ax1,read_new_01Zsun_05pc,70, 380, Cell,'G9_Zlow_HR','g',False,220)
main(ax1,read_new_gasrich,150, 300, Cellfromdat,'G9_Zlow_gas5','b',False,300)
...
ax1.legend()
#plt.show()
plt.savefig('/Volumes/THYoo/kisti/plot/2019thesis/ks_fig2_all2.png')
