...
def jonghab(ax1,ax2,cax1,cax2,read, inisnap, endsnap, zbin, zselect, zval, load,markerset): # zbin indicates the center of each z bin, not an edge.
    ...
    cm2 = plt.get_cmap('summer')
    vmin1 = np.min(outflowarr2[np.where(outflowarr2>0)]) ; vmax1=np.max(outflowarr2)
    vmin2 = np.min(recratearr2[np.where(recratearr2>0)]); vmax2 =np.max(recratearr2)
    for k in range(len(zselect)):
        ss=ax1.scatter(HIdenarr2[:,k],fescarr, marker=markerset[k],c=outflowarr2[:,k],s=2,cmap=cm1, norm=colors.LogNorm(vmin=vmin1,vmax=vmax1))
        ss2=ax2.scatter(HIdenarr2[:,k],fescarr, marker=markerset[k],c=recratearr2[:,k],s=2,cmap=cm2,norm=colors.LogNorm(vmin=vmin2,vmax=vmax2 ))
    cbar1 = plt.colorbar(ss, cax=cax1, cmap=cm1)
    cbar1.set_label('$dM_{out}/dt$ $(M_\odot/yr)$')

    cbar2 = plt.colorbar(ss2, cax=cax2, cmap=cm2)
    cbar2.set_label(r'$dN_{HI}/dt$ $(cm^{-3}\cdot s^{-1})$')

    ax1.set_xlabel('nHI')
    ax1.set_ylabel('$f_{esc}$')
    ax2.set_xlabel('nHI')
    ax2.set_ylabel('$f_{esc}$')
    ax1.set_xscale('log')
    ax2.set_xscale('log')
    ax1.set_xlim(8e-3,5)
    ax1.set_ylim(3e-4,0.1)
    ax2.set_xlim(8e-3, 5)
    ax2.set_ylim(3e-4, 0.1)
    #ax2.set_yticks([])
...
ax2 = fig.add_axes([0.5, 0.1, 0.25, 0.3])
ax3 = fig.add_axes([0.1, 0.5, 0.25, 0.3])
ax4 = fig.add_axes([0.5, 0.5, 0.25, 0.3])
cax1 = fig.add_axes([0.35,0.1,0.01,0.3])
cax2 = fig.add_axes([0.75,0.1,0.01,0.3])
cax3 = fig.add_axes([0.35,0.5,0.01,0.3])
cax4 = fig.add_axes([0.75,0.5,0.01,0.3])

#zbin = np.linspace(-4000, 4000, num=16)

#jonghab(ax1,ax2,ax3,read_new_01Zsun,130,331,th,zbin,'olive','G9_01Zsun')
markerset=['o','s','p','x','D','*']
rec1 = jonghab(ax1,ax2,cax1,cax2,read_new_1Zsun,130,481,np.linspace(100,8000,num=80),[0,4,9,19,39,-1], 4000, True, markerset)
rec2 = jonghab(ax3,ax4,cax3,cax4,read_new_1Zsun_highSN_new,30,380,np.linspace(100,8000,num=80),[0,4,9,19,39,-1], 4000, True, markerset)
