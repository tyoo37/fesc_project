...
def fesc_rel(ax, ax2, read, inisnap, endsnap, Fesc, label, loadsave):
   ...
   # print(np.median(attarr)-np.percentile(attarr,50-18.2689),np.percentile(attarr,68.2689)-np.median(attarr))
    ...
    ax.text(480, 0.08, label, ha='right')

    ax2.text(480, 0.48, label, ha='right')
...
ax4 = fig.add_axes([0.6, 0.1, 0.32, 0.26])
ax5 = fig.add_axes([0.6, 0.4, 0.32, 0.26])
ax6 = fig.add_axes([0.6, 0.7, 0.32, 0.26])


ax44 = fig.add_axes([0.13, 0.1, 0.32, 0.26])
ax55 = fig.add_axes([0.13, 0.4, 0.32, 0.26])
ax66 = fig.add_axes([0.13, 0.7, 0.32, 0.26])
...
fesc_rel(ax66,ax6, read_new_01Zsun, 150, 480, Fesc_new8,'G9_Zlow',True)
#fesc_rel(ax11,ax1, read_new_01Zsun_05pc, 150, 380, Fesc_new, 'G9_Zlow_HR',False)
fesc_rel(ax55,ax5, read_new_1Zsun, 150, 480, Fesc_new8,  'G9_Zhigh',True)
#fwesc_rel(ax22,ax2, read_new_1Zsun_highSN_new, 70, 380, Fesc_new8,  'G9_Zhigh_SN5',False)
#fesc_rel(ax33,ax3, read_new_03Zsun_highSN, 70, 380, Fesc_new8,  'G9_Zmid_SN5',False)
fesc_rel(ax44,ax4, read_new_gasrich, 150, 300, Fesc_new8, 'G9_Zlow_gas5',True)
...
ax4.set_xlabel('time (Myr)',fontsize=23)
ax44.set_xlabel('time (Myr)',fontsize=23)
...
fig.text(0.03,0.4,'$(F_{900}/F_{1500})_{ISM+CGM}$',fontsize=23,rotation=90)
...
print(0.008/0.27)
plt.savefig('/Volumes/THYoo/kisti/plot/2019thesis/fig11_thesis1.pdf')
