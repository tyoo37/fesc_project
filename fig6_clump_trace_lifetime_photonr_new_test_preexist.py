...
read_new_01Zsun_lyaoff = '/Volumes/gdrive/0.1Zsun_lyaoff_highSN/'
class Part():
...
def ax_plot(load, Fesc_new,read, inisnap, endsnap, max_trace, preexist, pdf, color, label, ls,verbose,lw):
    if load==True:
        if preexist==False:
            ...
            lifetimearr2 = np.loadtxt(read+'lifetimearr2.dat')
            print(np.mean(lifetimearr2))
        ...
        if preexist ==False:
            ...
            lifetimearr2, photonrarr2, fracarr2 = lifetime_in_clump(Part,Clump,Fesc_new,YoungClumpFind,read,inisnap,endsnap,max_trace,preexist,verbose)
            print(lifetimearr2.shape)
            print(fracarr2.shape)
            print(fracarr2)
            print(np.min(lifetimearr2), np.max(lifetimearr2))
            print('meanlifetime', np.mean(lifetimearr2))
            np.savetxt(read + 'lifetimearr2.dat', lifetimearr2, newline='\n', delimiter=' ')
            np.savetxt(read + 'photonrarr2.dat', photonrarr2, newline='\n', delimiter=' ')
            np.savetxt(read + 'fracarr2.dat', fracarr2, newline='\n', delimiter=' ')
            maxpop = lifetimearr2[lifetimearr2 == 41]
    ...
    if preexist==False:
        hist, binedge = np.histogram(lifetimearr, bins=41, range=(0,41),density=pdf)
    else:
        hist2, binedge = np.histogram(lifetimearr2, bins=41, range=(0,41),density=pdf)
    ...
    if preexist==False:
        ...
        ax2.plot(mid, hist2, marker='o', color=color, lw=lw, label=label)
...
ax1 = fig.add_axes([0.15, 0.55, 0.7, 0.35])
...
ax2.set_ylabel('PDF')
ax2.set_xticks(np.linspace(0, 20, num=11))
#ax4.set_xlabel('# of companions')
ax_plot(True, Fesc_new8, read_new_01Zsun,150,480,40,False,True, 'k','G9_Zlow','solid',False,2)
ax_plot(True, Fesc_new8, read_new_1Zsun,150,480,40,False,True,'r','G9_Zhigh','solid',False,2)
ax_plot(True,Fesc_new8, read_new_gasrich,150,300,40,False,True, 'b','G9_Zlow_gas5','solid',False,2)
ax_plot(True, Fesc_new8,read_new_03Zsun_highSN,70,380,40,False,True,'orange','G9_Zmid_SN5','solid',False,2.5)
ax_plot(True, Fesc_new8, read_new_1Zsun_highSN_new,70,380,40,False,True,'magenta','G9_Zhigh_SN5','dashed',False,2.5)
ax_plot(True, Fesc_new8, read_new_01Zsun_lyaoff,150,438,40,False,True,'lightseagreen','G9_Zlow_lyaoff','dashed',False,2.5)

print(' ')
ax_plot(True, Fesc_new8, read_new_01Zsun,150,480,40,True,True, 'k','G9_Zlow','solid',True,2)
ax_plot(True, Fesc_new8, read_new_1Zsun,150,480,40,True,True,'r','G9_Zhigh','solid',True,2)
ax_plot(True,Fesc_new8, read_new_gasrich,150,300,40,True,True, 'b','G9_Zlow_gas5','solid',True,2)
ax_plot(True, Fesc_new8,read_new_03Zsun_highSN,70,380,40,True,True,'orange','G9_Zmid_SN5','solid',True,2.5)
ax_plot(True, Fesc_new8, read_new_1Zsun_highSN_new,70,380,40,True,True,'magenta','G9_Zhigh_SN5','dashed',True,2.5)
ax_plot(True, Fesc_new8, read_new_01Zsun_lyaoff,150,438,40,True,True,'lightseagreen','G9_Zlow_lyaoff','dashed',False,2.5)
...
#plt.savefig('/Volumes/THYoo/kisti/plot/2019thesis/fig6_thesis.pdf')
