...
def ax_plot(load, ax, ax2,ax3,Fesc_new,read, inisnap, endsnap, max_trace, preexist, pdf, color, label, ls,verbose):
    ...
    ax.hist(lifetimearr, bins=41, range=(0,41),density=pdf, histtype='stepfilled',edgecolor=color, label=label, facecolor="None",ls='solid')
    #ax2.plot(np.linspace(0,19,num=20)+0.5,np.log10(photonrarr),color=color,ls='dashed')
    ax3.plot(np.linspace(0,39,num=40)+0.5,fracarr,color=color,ls=ls,marker='s')
    ax2.plot(np.linspace(0,39,num=40)+0.5,cumfracarr,color=color,ls='dotted')
    ax3.set_ylim(0,0.3)
    ax2.set_ylim(0,1)
...
snap=[151,152,155,160,180,200,300]
snap2=[151,152,155,160,180,200,480]
...
for i in range(7):
    ...
    ax3 = ax2.twinx()
    ax3.set_ylabel('$(\sum \dot N(t))(t)$')
    #ax4.set_xlabel('# of companions')
    ax_plot(False, ax1,ax3,ax2,Fesc_new8, read_new_1Zsun,150,480,40,False,True,'r','G9_Zhigh','solid',False)
    ax_plot(False, ax1,ax3,ax2,Fesc_new8, read_new_01Zsun,150,snap2[i],40,False,True, 'grey','G9_Zlow','solid',False)
    ax_plot(False, ax1,ax3,ax2,Fesc_new8, read_new_gasrich,150,snap[i],40,False,True, 'b','G9_Zlow_gas5','dotted',False)
    ax_plot(True, ax1,ax3,ax2,Fesc_new8,read_new_03Zsun_highSN,70,380,40,False,True,'orange','G9_Zmid_SN5','dashed',False)
    ax_plot(True, ax1,ax3,ax2,Fesc_new8, read_new_1Zsun_highSN_new,70,380,40,False,True,'khaki','G9_Zhigh_SN5','dashed',False)
    #plot6, lifetimearr6 = ax_plot(True, ax1,read_new_03Zsun_highSN,25,280,20,5,'lifetimearr_03Zsun_SNen.dat',True,'g','G9_03Zsun_SNboost','solid')
    ...
    plt.savefig('/Volumes/THYoo/kisti/plot/2019thesis/fig6_sanity_%d.pdf'%i)
    plt.close()
