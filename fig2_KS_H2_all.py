...
def scatter_kennicutt(ax,filename,colnum,color,sfrind,Hind,marker,label):
    arr = fits_to_arr(colnum,filename)
    SFR = 10**arr[:,sfrind].astype(float)
    H = 10**arr[:,Hind].astype(float) * 1.67e-24 * 2 * 3.08e18 /1.98e33 *3.08e18
...
def draw_contour(ax, xarr, yarr,color,label):
    xx, yy = np.mgrid[np.min(np.log10(xarr))-0.5:np.max(np.log10(xarr))+0.5:200j, np.min(np.log10(yarr))-0.5:np.max(np.log10(yarr))+0.5:200j]
    positions = np.vstack([xx.ravel(), yy.ravel()])
    values = np.vstack([np.log10(xarr), np.log10(yarr)])
    kernel = st.gaussian_kde(values)
    f = np.reshape(kernel(positions).T, xx.shape)
    sigma_1, sigma_2, sigma_3 = sigma3(f)
    ss3 = ax.contour(np.power(10,xx),np.power(10,yy),f,levels=[sigma_1,sigma_2,sigma_3],colors=color,linewidths=2,origin='lower',alpha=0.8,label=label)
...
def main(ax, read, inisnap, endsnap, Cell,label,color, saveload,diskmass):
    ...
    if saveload==False:
        ...
        for i in range(numsnap):
            ...
            halfmass_sd, halfmass_sfrd, halfmassrad = halfmassrad_KS(Part1, Cell1, xcen, ycen, zcen, 8000,hmr)
    ...
    ax.scatter(sdarr, sfrdarr, color=color, marker='o', s=8, alpha=0.5,label=label)
    print(sdarr, sfrdarr)
...
scatter_kennicutt(ax1,'/Users/taehwau/projects/KS/kennicutt07_total.fit',4,'grey',0,2,'x', 'Kennicutt+07 (M51)')
scatter_tacconi(ax1,'/Users/taehwau/projects/KS/tacconi13_2.fit',2,'grey',1,0,'^', 'Tacconi+13')

#scatter2(ax1,'/Users/taehwau/projects/KS/tacconi13.fit',5,'grey','grey',4,3,'^', 's','Tacconi+13 (z<2)','Tacconi+13 (z>2)')

main(ax1,read_new_01Zsun,150, 250, Cell,'G9_Zlow','k',False,300)
main(ax1, read_new_1Zsun,150, 250, Cell,'G9_Zhigh','r',False,300)
main(ax1,read_new_1Zsun_highSN_new,70, 170, Cell,'G9_Zhigh_SN5','magenta',False,220)
main(ax1,read_new_03Zsun_highSN,70, 170, Cell,'G9_Zmid_SN5','orange',False,220)
main(ax1,read_new_01Zsun_05pc,70, 170, Cell,'G9_Zlow_HR','g',False,220)
main(ax1,read_new_gasrich,150, 250, Cellfromdat,'G9_Zlow_gas5','b',False,300)

ax1.set_xlabel(r'$\Sigma_{H2}$ $(M_\odot/pc^2)$',fontsize=20)
