...
def fescplot(ax, ax2, Part, Fesc, dir, inisnap, endsnap,  label,  load, color, color2, text, legend):
    ...
    err = bootstrap(fescarray[index2], photonrarr[index2],100,100)
    print('err',err)
...
def bootstrap(arr,wei,samplesize,numiter):
    numiter = int(len(arr)*np.log(len(arr))**2)
    print(numiter)
    rdmeanarr = [];rdmeanarr2 = []
    for n in range(numiter):
        ind = np.arange(len(arr))
        rdind = random.choices(list(ind), k=samplesize)
        rdmean = np.sum(arr[rdind]*wei[rdind])/np.sum(wei[rdind])
        rdmeanarr.append(rdmean)
        rdmeanarr2.append(rdmean**2)

    btmean = np.sum(rdmeanarr)/numiter
    btmean2 = np.sum(rdmeanarr2)/numiter


    rdmeanstd = np.sqrt(numiter/(numiter-1)*(btmean2-btmean**2))
...
ax1 = fig.add_axes([0.09, 0.3, 0.39, 0.21])
ax2 = fig.add_axes([0.52, 0.3, 0.39, 0.21])
ax3 = fig.add_axes([0.09, 0.54, 0.39, 0.21])
ax4 = fig.add_axes([0.52, 0.54, 0.39, 0.21])
ax5 = fig.add_axes([0.09, 0.78, 0.39, 0.21])
ax6 = fig.add_axes([0.52, 0.78, 0.39, 0.21])

ax7 = fig.add_axes([0.09,0.05, 0.82,0.18])
...
fesc1,err1 = fescplot(ax5, ax55, Part, Fesc_new8, read_new_01Zsun, 2, 480, 'G9_Zlow',True, 'dodgerblue','r',True,True)
...
fesc2,err2 =fescplot(ax2, ax22, Part, Fesc_new8, read_new_1Zsun_highSN_new, 2,380,  'G9_Zhigh_SN5',True,'dodgerblue', 'r',  True, False)
fesc3,err3 =fescplot(ax4, ax44, Part, Fesc_new8, read_new_03Zsun_highSN, 2,380, 'G9_Zmid_SN5',True,'dodgerblue', 'r',  True, False)

fesc4,err4 =fescplot(ax6, ax66, Part, Fesc_new, read_new_01Zsun_05pc, 2,380, 'G9_Zlow_HR',True,'dodgerblue', 'r',  True, False)
fesc5, err5 =fescplot(ax3, ax33, Part, Fesc_new8, read_new_gasrich, 2, 300, 'G9_Zlow_gas5',True,'dodgerblue', 'r',  True, False)
fesc6, err6 =fescplot(ax1, ax11, Part, Fesc_new8, read_new_1Zsun, 2, 480, 'G9_Zhigh',True,'dodgerblue', 'r',  True, False)
...
ax7.scatter(barx, bary, color=color, s=30, marker='s')
for i in range(6):
    print(barx[i],bary[i], errset[i],color[i])
    ax7.errorbar(barx[i],bary[i],yerr=np.array([[errset[i]]]).T,fmt='o',color=color[i],ecolor=color[i],elinewidth=2,capsize=8 )
