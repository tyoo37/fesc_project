...
def plot_scatter(ax, ax2, ax3,Cell, Fesc, read, inisnap, endsnap, color, label, loadcom, load):
    ...
    if load==False:
        ...
        np.savetxt(read+'sdarr22.dat',sdarr2,newline='\n',delimiter=' ')

    else:
        sfrdarr = np.loadtxt(read+'sfrd_fesc.dat')
        fescarr = np.loadtxt(read+'fesc_sfrd.dat')
        sdarr2 = np.loadtxt(read+'sdarr22.dat')
...
#    sdarr2 = np.loadtxt(read + 'sdarr2.dat')
    ax3.scatter(sdarr2, fescarr, color=color, s=2)
    plot_in_scatter(ax3, sdarr2, fescarr, color, label)
...
fig = plt.figure(figsize=(8, 18), dpi=144)

ax1 = fig.add_axes([0.15, 0.7, 0.8, 0.22])
ax2 = fig.add_axes([0.15,0.1,0.8,0.22])
ax3 = fig.add_axes([0.15,0.4,0.8,0.22])
plot_scatter(ax1,ax2,ax3,Cell,Fesc_new8,read_new_01Zsun,150,480,'k','G9_Zlow',True,False)
plot_scatter(ax1,ax2,ax3,Cell,Fesc_new8,read_new_1Zsun,150,480,'firebrick','G9_Zhigh',True,False)
plot_scatter(ax1,ax2,ax3,Cellfromdat,Fesc_new8,read_new_gasrich,150,300,'dodgerblue','G9_Zlow_gas5',True,False)
plot_scatter(ax1,ax2,ax3,Cell,Fesc_new8,read_new_03Zsun_highSN,70,380,'orange','G9_Zmid_SN5',True,False)
plot_scatter(ax1,ax2,ax3,Cell,Fesc_new8,read_new_1Zsun_highSN_new,70,380,'magenta','G9_Zhigh_SN5',True,False)
...
ax3.set_xscale('log')
ax3.set_yscale('log')
ax3.set_ylabel('$log(f_{esc})$')
ax3.set_xlabel('r$\Sigma_{H2}$ $(M_\odot/pc^2)$')
#plt.savefig('/Volumes/THYoo/kisti/plot/2019thesis/sfrd_fesc_fin_thesis.pdf')
plt.show()