...
def plot_double(ax1, ax2, read, nout, Part, Cell, xwid, ywid, zwid, label,diskmass):
...
def single(read,nout,Cell,savedirec,dpi,label):

    start = time.time()

    fig = plt.figure(figsize=(5*3, 5*3),dpi=dpi)
...
#    Fesc_indSED1 = Fesc_indSED(read, nout)
    ...
    a = new_projection(Cell1, xcen, ycen, zcen, 7000,3500, 7000, 'xHI', 'xz')
    cm1 = plt.get_cmap('inferno')
    cm3 = plt.get_cmap('pink')
    a.projectionPlot(ax1,cm1,[-5,-4,-3,-2,-1,0],True,True,1,True,label,'w')
   ...
   # d.projectionPlot(ax2, cm3, [-4, -2,  0,2],True,True,1,False,label,'w')
    ...
    #plt.savefig(savedirec+'thr_fiducial_%s_%05d.png'%(label,nout))
    plt.show()
single(read_new_01Zsun,480,Cell,'/Volumes/THYoo/kisti/plot/2019thesis/',72,'Zlow_xHI')
