...
class Fesc_new8():
    def __init__(self, dir, nout):
        ...
        self.fescD = dat2.read_reals(dtype='float32')
        self.photonr = dat2.read_reals(dtype=np.double)
        print('photonr',np.sum(self.photonr))
        self.fesc = np.sum(self.fescD * self.photonr) / np.sum(self.photonr)
        self.fesc2 = np.sum(self.fescH * self.photonr) / np.sum(self.photonr)
...
def fesc_rel_snapshot(Fesc1, edge900, edge1500):
    ...
    return int900/int1500, fescrel, att900/att1500, att900/int900, att1500/int1500, att900, int900, att1500, int1500
...
def fesc_rel(ax, ax2, read, inisnap, endsnap, Fesc, label, loadsave):
    ...
    if loadsave==False:
        ...
        attarr = [];attarr2=[]; att900arr=[]; FL1500arr=[]; att1500arr=[]
        prev=0
        ...
        for n in range(numsnap):
            ...
            M17intratio, M17fescrel, M17attratio, FL900M17, FL1500M17, att900M17, int900M17,att1500M17,int1500M17 = fesc_rel_snapshot(Fesc1, M17edge900, M17edge1500)
            S18intratio, S18fescrel, S18attratio, FL900S18, FL1500S18, att900S18, int900S18,att1500S18,int1500S18 = fesc_rel_snapshot(Fesc1, S18edge900, S18edge1500)
        ...
        np.savetxt(read+'att1500arr.dat',att1500arr,newline='\n',delimiter=' ')
        ...
        att1500arr = np.loadtxt(read+'att1500arr.dat')
    ...
    print('median value of F1500/L1500'+label, np.median(FL1500arr),onesigerr(FL1500arr))
   # print(np.median(attarr)-np.percentile(attarr,50-18.2689),np.percentile(attarr,68.2689)-np.median(attarr))
    f900lum = np.sum(att900arr*FL900arr)/np.sum(att900arr)
    f1500lum = np.sum(att1500arr*FL1500arr)/np.sum(att1500arr)
...
plt.rcParams['hatch.linewidth'] = 1.5 # previous pdf hatch linewidth

fig = plt.figure(figsize=(11,11))

ax1 = fig.add_axes([0.6, 0.1, 0.32, 0.13])
ax2 = fig.add_axes([0.6, 0.25, 0.32, 0.13])
ax3 = fig.add_axes([0.6, 0.4, 0.32, 0.13])
ax4 = fig.add_axes([0.6, 0.55, 0.32, 0.13])
ax5 = fig.add_axes([0.6, 0.7, 0.32, 0.13])
ax6 = fig.add_axes([0.6, 0.85, 0.32, 0.13])

ax11 = fig.add_axes([0.13, 0.1, 0.32, 0.13])
ax22 = fig.add_axes([0.13, 0.25, 0.32, 0.13])
ax33 = fig.add_axes([0.13, 0.4, 0.32, 0.13])
ax44 = fig.add_axes([0.13, 0.55, 0.32, 0.13])
ax55 = fig.add_axes([0.13, 0.7, 0.32, 0.13])
ax66 = fig.add_axes([0.13, 0.85, 0.32, 0.13])
...
for ax in ax2list:
    ax.plot([0, 500],[0.008/0.27,0.008/0.27], c='c',lw=2)
    ax.fill_between([0, 500],[0.008/0.27-0.04/3,0.008/0.27-0.04/3],[0.008/0.27+0.04/3,0.008/0.27+0.04/3],facecolors="none",alpha=0.4,edgecolor='c',hatch='///')
    ax.plot([0, 500],[0.047,0.047], c='tomato',lw=2)
    ax.fill_between([0, 500],[0.047-0.005,0.047-0.005],[0.047+0.005,0.047+0.005],facecolors='none',edgecolor='tomato',alpha=0.4,hatch='\\\\\\\\\\',)
for ax in axlist:
    ax.plot([0, 500],[0.33,0.33], c='c',lw=2)
    ax.plot([0, 500],[0.27,0.27], c='tomato',lw=2)

fesc_rel(ax66,ax6, read_new_01Zsun, 3, 480, Fesc_new8,'G9_Zlow',False)
fesc_rel(ax11,ax1, read_new_01Zsun_05pc, 3, 380, Fesc_new, 'G9_Zlow_HR',False)
fesc_rel(ax55,ax5, read_new_1Zsun, 3, 480, Fesc_new8,  'G9_Zhigh',False)
fesc_rel(ax22,ax2, read_new_1Zsun_highSN_new, 3, 380, Fesc_new8,  'G9_Zhigh_SN5',False)
fesc_rel(ax33,ax3, read_new_03Zsun_highSN, 3, 380, Fesc_new8,  'G9_Zmid_SN5',False)
fesc_rel(ax44,ax4, read_new_gasrich, 3, 300, Fesc_new8, 'G9_Zlow_gas5',False)
...
fig.text(0.03,0.4,'$(F_{900}/F_{1500})_{ISM+CGM}^{3D}$',fontsize=23,rotation=90)
fig.text(0.5,0.47,'$L_{900}/L_{1500}$',fontsize=23,rotation=90)
