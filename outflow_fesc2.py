...
read_new_1Zsun_SNboost_re = '/blackwhale/dbahck37/kisti/1Zsun_SNen_new/'

read_new_03Zsun_SNboost_re = '/blackwhale/dbahck37/kisti/0.3Zsun_SNen/'

read_new_5pc_re = '/blackwhale/dbahck37/kisti/0.1Zsun_5pc/'
...
def getoutflow(Cell, zz, up, xcen, ycen, rad):
    if up == True:
        ind = np.where((Cell.z + Cell.dx / 2 > zz) & (Cell.z - Cell.dx / 2 < zz) & (Cell.vz > 0) & (
                    get2dr(Cell.x, Cell.y, xcen, ycen) < rad))
    else:
        ind = np.where((Cell.x + Cell.dx / 2 > zz) & (Cell.x - Cell.dx / 2 < zz) & (Cell.vz < 0) & (
                    get2dr(Cell.x, Cell.y, xcen, ycen) < rad))
...
def getoutflow_box(Cell, zz, up, xcen, ycen):
    if up == True:
        ind = np.where((Cell.z + Cell.dx / 2 > zz) & (Cell.z - Cell.dx / 2 < zz) & (Cell.vz > 0))
    else:
        ind = np.where((Cell.z + Cell.dx / 2 > zz) & (Cell.z - Cell.dx / 2 < zz) & (Cell.vz < 0))
...
def plot_in_scatter(ax, x, y, color, label):
    xbin = [np.min(x), np.percentile(x, 20), np.percentile(x, 40), np.percentile(x, 60), np.percentile(x, 80),
            np.max(x)]
    ...
    xxbin = [np.percentile(x, 10), np.percentile(x, 30), np.percentile(x, 50), np.percentile(x, 70),
             np.percentile(x, 90)]
...
def main(read, Cell, Fesc, inisnap, endsnap, load, diskmass, zbin, axlist, axlist2, color, label, loadcom):
    ...
    if load == False:
        ...
        for n in range(numsnap):
            ...
            if loadcom == False:
                xcen, ycen, zcen, hmr = CoM_Main(Part1, Cell1, diskmass)
        ...
        np.savetxt(read + 'outflowarr_box_of.dat', outflow_boxarr2, newline='\n', delimiter=' ')
        ...
        outflow_boxarr2 = np.loadtxt(read + 'outflowarr_box_of.dat')

    for j in range(len(zbin)):
        axlist[j].scatter(outflowarr2[:, j], fescarr, color=color, label=label, s=0.5)
        axlist2[j].scatter(outflow_boxarr2[:, j], fescarr, color=color, label=label, s=0.5)
        plot_in_scatter(axlist[j], outflowarr2[:, j], fescarr, color, label)
        plot_in_scatter(axlist2[j], outflow_boxarr2[:, j], fescarr, color, label)
...
ax4 = fig.add_axes([0.1, 0.5, 0.35, 0.35])
ax5 = fig.add_axes([0.5, 0.5, 0.35, 0.35])
...
ax1.set_xlabel('$(dm_{out, 2 kpc}/dt) (M_\odot/yr)$')
ax2.set_xlabel('$(dm_{out, 10 kpc}/dt) (M_\odot/yr)$')
# ax3.set_xlabel('$(dm_{out, 2000pc}/dt)_{starhmr} (M_\odot/yr)$')
ax4.set_xlabel('$(dm_{out, 2 kpc}/dt)_{box} (M_\odot/yr)$')
ax5.set_xlabel('$(dm_{out, 10 kpc}/dt)_{box} (M_\odot/yr)$')
