...
class Part():
    def __init__(self, dir, nout):
        self.nout = nout
        partdata = readsav(dir + '/SAVE/part_%05d.sav' % (self.nout))
        # time of snapsnot in units of Myr
        self.snaptime = partdata.info.time*partdata.info.unit_t/365/3600/24/1e6

        pc = 3.08e18
        # simulation box size in units of pc
        self.boxpc = partdata.info.boxlen * partdata.info.unit_l / pc
        self.boxlen = partdata.info.boxlen
        xp = partdata.star.xp[0]

        # the position of stellar particle in units of pc
        ...
        # stellar particle age in units of Myr
        self.starage = self.snaptime - partdata.star.tp[0]*partdata.info.unit_t/365/3600/24/1e6

        # stellar particle ID
        self.starid = np.abs(partdata.star.id[0])

        # stellar particle mass in units of solar mass
        ...
        # stellar particle formation time
        tp = (partdata.info.time-partdata.star.tp[0]) * partdata.info.unit_t / 365. /24./3600/1e6
        sfrindex = np.where((tp >= 0) & (tp < 10))[0]

        # star formation rate (10 Myr average) (Msun / yr)
...
class Cell():
    def __init__(self, dir, nout):
        ...
        pc = 3.08e18
        self.boxpc = celldata.info.boxlen * celldata.info.unit_l / pc
        self.boxlen = celldata.info.boxlen
        self.unit_l = celldata.info.unit_l
        self.unit_d = celldata.info.unit_d


        self.nH = celldata.cell[0][4][0] * celldata.info.unit_nh
        self.x = celldata.cell.x[0] * self.boxpc
        self.y = celldata.cell.y[0] * self.boxpc
        self.z = celldata.cell.z[0] * self.boxpc
        self.dx = celldata.cell.dx[0] * self.boxpc
        ...
        mu = celldata.cell[0][4][0] * self.unit_d / 1.66e-24 / ntot
        self.m = celldata.cell[0][4][0] * self.unit_d * self.unit_l / 1.989e33 * self.unit_l *self.unit_l *(celldata.cell.dx[0]*self.boxlen)**3
...
class Cellfromdat():
    def __init__(self, dir, nout, Part):
        ...
        self.nH2 = var[:, 0] * 30.996344 * self.xH2
...
class new_projection:
    #xwid, ywid, zwid -> half width in pc scale, var-> variable ex) density -> nH, temperature ->T
    #projection -> 'xy' (face-on), 'xz' (edge-on)
...
#        print('minmax',minmax(xind_root),minmax(yind_root),minmax(zind_root))
    ...
    def projectionPlot(self, ax, cm, ticks,cbar,ruler,corr,text, label,color):
        start = time.time()
        ...
        if cbar==True:
            cbaxes = inset_axes(ax, width="100%", height="100%", loc=3, bbox_to_anchor=(0.09,0.08,0.35,0.03),bbox_transform=ax.transAxes)
            cbar = plt.colorbar(im, cax=cbaxes, ticks=ticks, orientation='horizontal', cmap=cm)
            cbar.set_label('log(' + self.var + ')', color=color, labelpad=-80, fontsize=25)
            cbar.ax.xaxis.set_tick_params(color=color,labelsize=25)
        ...
        if ruler==True:
            if self.projection == 'xy':
                ...
                rectangles = {
                    '5 kpc': patches.Rectangle(xy=(0.25 * self.xwid / 1000, -0.85 * self.ywid / 1000),
                ...
                for r in rectangles:
                    ...
                    ax.annotate(r, (cx, cy + 0.04 * (2 * self.ywid / 1000)*corr), color='w', weight='bold',
                                fontsize=30, ha='center', va='center')
            if self.projection == 'xz':
                rectangles = {
                    '5 kpc': patches.Rectangle(xy=(0.25 * self.xwid / 1000, -0.85 * self.zwid / 1000),
                ...
                for r in rectangles:
                    ...
                    ax.annotate(r, (cx, cy + 0.04 * (2 * self.zwid / 1000) * corr), color='w', weight='bold',
                                fontsize=30, ha='center', va='center')
        if text==True:
            if self.projection=='xy':
    ...
    def star_plot3(self, Part, ax,binsize,cm,ticks,vmin,vmax, cbar, ruler,corr):
        ...
        if cbar==True:
            cbaxes = inset_axes(ax, width="100%", height="100%", loc=3, bbox_to_anchor=(0.09,0.08,0.35,0.03),
                                bbox_transform=ax.transAxes)
            cbar = plt.colorbar(im, cax=cbaxes, ticks=ticks, orientation='horizontal', cmap=cm)
            cbar.set_label('$log(\Sigma_*) (M_\odot\cdot kpc^{-2}$)', color='w', labelpad=-80, fontsize=25)
            cbar.ax.xaxis.set_tick_params(color='w',labelsize=25)
            cbar.ax.xaxis.set_ticks_position('bottom')
        ...
        if ruler==True:
            if self.projection =='xy':
                ...
                rectangles = {
                    '5 kpc': patches.Rectangle(xy=(0.25 * self.xwid / 1000, -0.85 * self.xwid / 1000),
                                               width=(2 * self.xwid / 1000) * 5 / 14,
                                               height=0.01 * (2 * self.ywid / 1000)*corr, facecolor='white')}
                for r in rectangles:
                    ax.add_artist(rectangles[r])
...
#    Fesc_indSED1 = Fesc_indSED(read, nout)
    ...
    a.projectionPlot(ax1,cm1,[-3,-2,-1,0,1,2],True,True,1,True,label,'w')
    b = new_projection(Cell1, xcen, ycen, zcen, 7000, 7000, 100, 'xHI', 'xy')
    b.projectionPlot(ax2,cm2,[-2, -1,0],True,True,1,False,label,'w')
    ...
    c.projectionPlot(ax5, cm1, [-3, -2, -1, 0, 1, 2],False,True,2,False,label,'k')
    c.star_plot3(Part1,ax4,a.mindx,cm3,[6,7,8],5.5,8.5,False,True,2)
    #plt.savefig(savedirec+'thr_fiducial%s_%05d_%d.pdf'%(label,nout,dpi))
    plt.show()
...
#    Fesc_indSED1 = Fesc_indSED(read, nout)
    ...
    a.star_plot3(Part1,ax3,a.mindx,cm3,[6,7,8],5.5,8.5,True,True,1)
    c = new_projection(Cell1, xcen, ycen, zcen, 7000, 7000, 3500, 'nH', 'xz')
    c.projectionPlot(ax5, cm1, [-3, -2, -1, 0, 1, 2],False,True,2,False,label,'w')
    c.star_plot3(Part1,ax4,a.mindx,cm3,[6,7,8],5.5,8.5,False,True,2)
    d = new_projection(Cell1, xcen, ycen, zcen, 7000, 7000, 3500, 'xH2', 'xy')

    d.projectionPlot(ax6, cm4, [-3, -2, -1, 0],True,True,1,False,label,'k')
    plt.savefig(savedirec+'thr_fiducial_%s_%05d.png'%(label,nout))


def thr_fiducial_mol2(read,nout,Cell,savedirec,dpi,label):

    start = time.time()

    fig = plt.figure(figsize=(10*3, 5*3),dpi=dpi)
...
#    Fesc_indSED1 = Fesc_indSED(read, nout)
    ...
    a = new_projection(Cell1, xcen, ycen, zcen, 7000,7000, 3500, 'nH', 'xy')
    cm1 = plt.get_cmap('inferno')
    cm3 = plt.get_cmap('pink')
    a.projectionPlot(ax1,cm1,[-3,-2,-1,0,1,2],True,True,1,True,label,'w')

    d = new_projection(Cell1, xcen, ycen, zcen, 7000, 7000, 3500, 'nH2', 'xy')

    d.projectionPlot(ax2, cm3, [-4, -2,  0,2],True,True,1,False,label,'w')
    for i in range(8):
...
thr_fiducial(read_new_01Zsun,480,Cell,'/Volumes/THYoo/kisti/plot/2019thesis/',72,'G9_Zlow')
#thr_fiducial_mol2(read_new_gasrich,300,Cellfromdat,'/Volumes/THYoo/kisti/plot/2019thesis/',72,'G9_Zlow_gas5')
