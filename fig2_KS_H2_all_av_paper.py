...
def scatter_tacconi(ax,filename,colnum,color,sfrind,Hind,marker,label):
    ...
    ax.scatter(H, SFR, color=color,marker=marker,label=label,alpha=0.5,s=40)
...
def main(ax, ax2, read, inisnap, endsnap, Cell, label, color, saveload):
    ...
    if read==read_new_01Zsun or read==read_new_gasrich or read==read_new_1Zsun:
        print(read, 1)
        noutind = np.where((noutarr>150)&(noutarr<300))
    else:
        print(read, 2)
        noutind = np.where((noutarr > 70) & (noutarr < 220))
    ...
    ax.errorbar(np.median(sdarr[noutind]), np.median(sfrdarr[noutind]), xerr=np.array([[xlow, xupp]]).T, yerr=np.array([[ylow, yupp]]).T,
    ...
    ax2.errorbar(np.median(sdarr2[noutind]), np.median(sfrdarr[noutind]), xerr=np.array([[xlow, xupp]]).T, yerr=np.array([[ylow, yupp]]).T,
                 marker='s', color='w', lw=6)

    ax2.errorbar(np.median(sdarr2[noutind]), np.median(sfrdarr[noutind]), xerr=np.array([[xlow, xupp]]).T, yerr=np.array([[ylow, yupp]]).T,
                 marker='s', color=color, lw=3)
    #print(sdarr, sfrdarr)
...
scatter_tacconi(ax2,'/Users/taehwau/projects/KS/tacconi13_2.fit',2,'grey',1,0,'^', 'Tacconi+13')#

main(ax1, ax2, read_new_01Zsun, 150, 480, Cell, 'G9_Zlow', 'k', True)
main(ax1,ax2, read_new_1Zsun,150, 480, Cell,'G9_Zhigh','r',True)
main(ax1,ax2,read_new_1Zsun_highSN_new,70, 380, Cell,'G9_Zhigh_SN5','magenta',True)
main(ax1,ax2,read_new_03Zsun_highSN,70, 380, Cell,'G9_Zmid_SN5','orange',True)
main(ax1,ax2,read_new_01Zsun_05pc,70, 380, Cell,'G9_Zlow_HR','g',True)
main(ax1,ax2,read_new_gasrich,150, 300, Cellfromdat,'G9_Zlow_gas5','b',True)
...
ax2.set_xlabel(r'$\Sigma_{H2}$ $(M_\odot/pc^2)$', fontsize=20)
ax2.set_ylabel(r'$\Sigma_*$ $(M_\odot/yr/kpc^2)$', fontsize=20)
ax2.set_xscale('log')
ax2.set_yscale('log')
ax2.set_xlim(1, 1e4)
ax2.set_ylim(10**(-3.5),10**1.5)
ax2.legend(frameon=False)



plt.savefig('/Volumes/THYoo/kisti/plot/2019thesis/ks_fig2_final.pdf')
plt.show()
# show me the money #